#import "../../../shared/typst/core.typ": paragraph, project-name, project-name-english, project-summary, project-type

= ВВЕДЕНИЕ

== Наименование программы

#paragraph[
 Наименование программы -- "#project-name()".
]

#paragraph[
 Наименование программы на английском языке -- "#project-name-english()".
]

== Документ(ы), на основании которого(ых) ведется разработка

#paragraph[
 Разработка ведется на основании учебного плана подготовки бакалавров по направлению 09.03.04 «Дизайн и разработка информационных продуктов», утвержденной темы курсового проекта и технического задания на разработку backend-части сервиса.
]
